...
oListBox = function(id, object_name, x, y, w, row_num, row_height, label, arr, selectedId, func, parentObject, parentPageId, offset) {
	...
	this.contextMenu = function(x, y, id) {
		...
		case (idx == 20):
			...
			p.list.groupby.push(new oGroupBy("copy of "+ c0, c1, c2, "Custom", c3, c4, c5, c6, c7, c8, c9, c10, c11));
			p.list.totalGroupBy++;
...
oPage = function(id, objectName, label, nbrows) {
	...
	this.init = function() {
		...
		case 2:
			...
			this.elements.push(new oTextBox(4, txtbox_x, Math.ceil(cSettings.topBarHeight + rh * 12.5), oTextBox_1, cHeaderBar.height, "Playlist filter ('*' = all playlists, 'null' = nothing, put ';' between playlists)", txtbox_value, "settings_textboxes_action", this.id));
			...
			this.elements.push(new oCheckBox(13, txtbox_x, cSettings.topBarHeight + rh * 19.5, "Visible", "p.list.groupby[p.settings.pages[2].elements[0].selectedId].showCover == 0 ? false : true", "settings_checkboxes_action", this.id));
			// Create checkbox Auto-Collpase ON/OFF
			this.elements.push(new oCheckBox(14, txtbox_x, cSettings.topBarHeight + rh * 21.5, "Enable", "p.list.groupby[p.settings.pages[2].elements[0].selectedId].autoCollapse == 0 ? false : true", "settings_checkboxes_action", this.id));

			var GHF_delta = 12.0;
			var txtbox_value = p.list.groupby[listBoxCurrentId].l1;
			this.elements.push(new oTextBox(15, txtbox_x, Math.ceil(cSettings.topBarHeight + rh * (13.5 + GHF_delta)), oTextBox_1, cHeaderBar.height, "Header line 1, left field", txtbox_value, "settings_textboxes_action", this.id));
			txtbox_value = p.list.groupby[listBoxCurrentId].r1;
			this.elements.push(new oTextBox(16, txtbox_x, Math.ceil(cSettings.topBarHeight + rh * (15.5 + GHF_delta)), oTextBox_1, cHeaderBar.height, "Header line 1, right field", txtbox_value, "settings_textboxes_action", this.id));
			txtbox_value = p.list.groupby[listBoxCurrentId].l2;
			this.elements.push(new oTextBox(17, txtbox_x, Math.ceil(cSettings.topBarHeight + rh * (17.5 + GHF_delta)), oTextBox_1, cHeaderBar.height, "Header line 2, left field", txtbox_value, "settings_textboxes_action", this.id));
			txtbox_value = p.list.groupby[listBoxCurrentId].r2;
			this.elements.push(new oTextBox(18, txtbox_x, Math.ceil(cSettings.topBarHeight + rh * (19.5 + GHF_delta)), oTextBox_1, cHeaderBar.height, "Header line 2, right field", txtbox_value, "settings_textboxes_action", this.id));
			this.elements.push(new oCheckBox(19, txtbox_x, cSettings.topBarHeight + rh * 33.5, "Show track count in line 2, right field (will override the upper setting, effective only when the header row height is 2)", ("l2_addinfo == true ? true : false"), "settings_checkboxes_action", this.id));
			// Create radio buttons for Defaul Group Status (Collapsed OR Expanded)
			var spaceBetween_w = zoom(90, zdpi);
			this.elements.push(new oRadioButton(20, txtbox_x, cSettings.topBarHeight + rh * 23.5, "Collapse", (p.list.groupby[p.settings.pages[2].elements[0].selectedId].collapseGroupsByDefault == "1"), "settings_radioboxes_action", this.id));
			this.elements.push(new oRadioButton(21, txtbox_x + spaceBetween_w, cSettings.topBarHeight + rh * 23.5, "Expand", (p.list.groupby[p.settings.pages[2].elements[0].selectedId].collapseGroupsByDefault == "0"), "settings_radioboxes_action", this.id));
			break;
		case 3:
			//foobox options
			var rh = cSettings.rowHeight;
			...
			this.elements.push(new oTextBox(5, txtbox_x, Math.ceil(cSettings.topBarHeight + rh * 8.75), oTextBox_3, cHeaderBar.height, "Cover filenames for groups by directory, put ';' between filenames", dir_cover_name, "settings_textboxes_action", this.id));			
			break;
		};
	};
	...
	this.draw = function(gr) {
		...
		case 2:
			...
			gr.GdiDrawText("Collapsed row height", g_font_b, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 15 - (this.offset * cSettings.rowHeight), txt_width, p.settings.lineHeight, lc_txt);
			gr.GdiDrawText("Expanded row height", g_font_b, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 16.75 - (this.offset * cSettings.rowHeight), txt_width, p.settings.lineHeight, lc_txt);
			gr.GdiDrawText("Cover", g_font_b, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 18.75 - (this.offset * cSettings.rowHeight), txt_width, p.settings.lineHeight, lc_txt);
			gr.GdiDrawText("Auto-collapse", g_font_b, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 20.75 - (this.offset * cSettings.rowHeight), txt_width, p.settings.lineHeight, lc_txt);
			gr.GdiDrawText("Default group status", g_font_b, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 22.75 - (this.offset * cSettings.rowHeight), txt_width, p.settings.lineHeight, lc_txt);
			gr.GdiDrawText("Group header fields", g_font_blank, p.settings.color2, txtbox_x, cSettings.topBarHeight + rh * 24.75 - (this.offset * cSettings.rowHeight), txt_width, p.settings.lineHeight, lc_txt);
			break;
		case 3:
	...
	this.newButtonPatternCheck = function(event, x, y) {
		...
		case "up":
			if (state == ButtonStates.hover) {
				// action
				p.list.groupby.push(new oGroupBy("Custom pattern", "null", "null", "Custom", "null", "2", "2", "1", "0", "-", "-", "-", "-", "0"));
				p.list.totalGroupBy++;
				window.SetProperty("SYSTEM.Groups.TotalGroupBy", p.list.totalGroupBy);
				var arr = [];
				fin = p.list.groupby.length;
...
oSettings = function() {
	...
	this.initpages = function(){
		if (this.pages.length <= 0) {
			...
			this.pages.push(new oPage(2, "p.settings.pages[2]", "Groups", 34));
