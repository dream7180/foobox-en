...
var m_x = 0, m_y = 0;
var g_seconds = 0;
var seek_len, seek_start,seek_h, vol_start, vol_len, pbo_start, btn_y, win_y, topbarh, topbtnw, menuicow, menulibw, menupbw, menubtnw,leftbarw, title_w, captionw, z4, z5, z8;
...
function detect_custom() {
	try{
		...
		custom_panel = 1;
		p_tips.push(window.GetProperty("custom.panel.tip", "Visualization"));
	} catch(e) {
		custom_panel = 0;
		p_tips.push("Settings");
	}
}
...
function initbuttons(){
	...
	if(show_menu) {
		MenubarBtn = [];
		for(var i = 0; i < 6; i++){
			if(i == 3) MenubarBtn.push(new ButtonUI(img_winbtn.Resize(menupbw,img_winbtn.Height,2)));
			else if(i == 4) MenubarBtn.push(new ButtonUI(img_winbtn.Resize(menulibw,img_winbtn.Height,2)));
...
function init_obj() {
	...
	if(show_menu) {
		...
		for(var i = 0; i < MenubarBtn.length; i++){
			...
			if(i == 3) _btnx += menupbw;
			else if(i == 4) _btnx += menulibw;
			else _btnx += menubtnw;
		}
		leftbarw = _btnx;
	} else {
...
function main_Menu(x, y) {
	...
	if(custom_panel){
		...
		basemenu.AppendMenuItem(MF_STRING, 1301, "foobox settings");
	}
...
function menu_bar(i) {
	...
	switch (i) {
		...
		case 4:
			...
			menuman1.BuildMenu(_menu, 1, 200);
			x = x + menubtnw * 3 + menupbw;
			break;
		case 5:
			menuman1.Init("help");
			menuman1.BuildMenu(_menu, 1, 200);
			x = x + menubtnw * 3 + menulibw + menupbw;
			break;
	}
	if(i == 0 && custom_panel){
		_menu.AppendMenuSeparator();
		_menu.AppendMenuItem(MF_STRING, 201, "foobox settings");
	}
...
function on_paint(gr) {
	...
	if(show_menu) {
		...
		gr.GdiDrawText("File", g_font, c_font, menuicow, 0, menubtnw, topbarh, cc_txt);
		gr.GdiDrawText("Edit", g_font, c_font, menuicow +  menubtnw, 0, menubtnw, topbarh, cc_txt);
		gr.GdiDrawText("View", g_font, c_font, menuicow +  menubtnw*2, 0, menubtnw, topbarh, cc_txt);
		gr.GdiDrawText("Playback", g_font, c_font, menuicow +  menubtnw*3, 0, menupbw, topbarh, cc_txt);
		gr.GdiDrawText("Library", g_font, c_font, menuicow +  menubtnw*3 + menupbw, 0, menulibw, topbarh, cc_txt);
		gr.GdiDrawText("Help", g_font, c_font, menuicow +  menubtnw*3 + menulibw + menupbw, 0, menubtnw, topbarh, cc_txt);
...
function on_mouse_lbtn_up(x, y) {
	...
	if(y < topbarh + 1) {
		...
		if (LibBtn.MouseUp()) {
			if(lib_albumlist) fb.RunMainMenuCommand("Library/Album List");
			else fb.RunMainMenuCommand("Library/ReFacets");
		}
