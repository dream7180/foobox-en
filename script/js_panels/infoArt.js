...
try{
	...
	console.log("Failed to create ESLyric interface, Option Tools->ESLyric->Advanced: pref.script.expose to be set to 1");
}
...
function Controller(imgArray, imgDisplay, prop) {
	...
	this.funcMenu = new function() {
		...
		this.Build = function() {
			...
			baseMenu.AppendMenuItem(currentPathItem && !currentPathItem.embed ? MF_STRING : MF_DISABLED, id++, "View in external application");
			...
			if (currentMetadb) {
				...
				subMenu_MAP.AppendMenuItem(MF_STRING, id++, "Artwork tag");
			...
			baseMenu.AppendMenuItem(show_infobar ? MF_STRING : MF_CHECKED, id++, "Hide track info");
			subMenu_ep.AppendTo(baseMenu, MF_POPUP, "Toggle lyrics and properties panel");
			funcs[id] = [on_mouse_mbtn_up, null];
			subMenu_ep.AppendMenuItem(MF_STRING, id++, "Switch (middle click)");
			funcs[id] = [toggle_autoswitch, null]
			subMenu_ep.AppendMenuItem(auto_eslprop ? MF_CHECKED : MF_STRING, id++, "Auto switch");
	...
	function SetMenuButtonCaption() {
		...
		var caption;
		if (currentPathItem) caption = currentPathItem.artId == -1 ? "Genre" : AlbumArtId.GetName(currentPathItem.artId).capitalize();
		else caption = "None";
		imgDisplay.menuButton.caption = caption;
	}
	...
	this.AboutCurrentImage = function() {
		...
		str.push("\nPicture type:  ");
		str.push(currentPathItem.artId == -1 ? "Genre" : AlbumArtId.GetName(currentPathItem.artId).capitalize());
...
function on_mouse_rbtn_up(x, y, vkey) {
	if (CoverDisplay.isXYIn(x, y)) MainController.funcMenu.Pop(x, y);
		...
		rMenu.AppendMenuItem(show_rating ? MF_STRING : MF_CHECKED, 7, "Hide rating");
		rMenu.AppendMenuItem(info_cycle ? MF_CHECKED : MF_STRING, 4, "Cycle track info");
		rMenu.AppendMenuSeparator();
		rMenu.AppendMenuItem(show_infobar ? MF_STRING : MF_CHECKED, 6, "Hide track info");
		rpSubMenu.AppendTo(rMenu, MF_POPUP, "Toggle lyrics and properties panel");
		rpSubMenu.AppendMenuItem(MF_STRING, 8, "Switch (middle click)");
		rpSubMenu.AppendMenuItem(auto_eslprop ? MF_CHECKED : MF_STRING, 9, "Auto switch");
		...
		case 1:
			...
			break;
		case 2:
			if (currentMetadb) fb.RunContextCommandWithMetadb("Properties", currentMetadb);
			break;
		case 3:
			is_mood = !is_mood;
		...
		if (currentMetadb) {
			...
			if (x > this.x && x < this.x + this.width && y > this.y && y < this.y + this.height) {
				...
				if (derating_flag) {
					...
					fb.RunContextCommandWithMetadb("Playback Statistics/Rating/" + "<not set>", currentMetadb);
				} else {
					if (rating_to_tag && tracktype < 2) handle_list.UpdateFileInfoFromJSON(JSON.stringify({"RATING" : i}));
					fb.RunContextCommandWithMetadb("Playback Statistics/Rating/" + i, currentMetadb);
				}
			}
		}
	}
