...
oHeaderBar = function() {
	...
	this.contextMenu = function(x, y, column_index, btnclk) {
		...
		_menu.AppendMenuSeparator();
		var _thislayout = "Default layout: ";
		if (layout.index != 0) _thislayout = "This layout: ";
		if(layout.uid == "Default layout") {
			_menu.AppendMenuItem(MF_STRING, 15, "Create new layout for this playlist");
			//_menu.AppendMenuSeparator();
		}
		_menu.AppendMenuItem(MF_STRING, 18, _thislayout + "Enable group");
		_menu.CheckMenuItem(18, layout.showgroupheaders);
		if (layout.showgroupheaders) {
			_patterns.AppendTo(_menu, MF_STRING, _thislayout + "Group pattern");
		...
		_columns.AppendTo(_menu, MF_STRING, _thislayout + "Columns");
