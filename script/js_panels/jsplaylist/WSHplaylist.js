			...
			if (this.ishover) {
				if (this.type == 1) { // group header
					...
					if (properties.defaultPlaylistItemAction != 0) {
						...
						fb.RunContextCommandWithMetadb("Add to playback queue", this.metadb, 0);
					};
				};
			};
			break;
		...
		if(track_edit_app != ""){
			if(g_track_type < 2){
				...
				if(_apps.length == 1){
					...
					_menu.AppendMenuItem(MF_STRING, 1012, "Open with "+arr[1]);
				} else {
					_child04.AppendTo(_menu, MF_STRING, "Open with...");
		...
		if (ret > 2 && ret < 800) {
			...
			case (ret > 1011 && ret < 1111):
				...
				if (g_track_type > -1) {
					...
					catch(e){fb.ShowPopupMessage("Application not found.", "Error Message");}
				} else{
