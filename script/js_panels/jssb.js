...
oBrowser = function() {
	...
	this.init_groups = function() {
		...
		for (var i = 0; i < total; i++) {
			...
			if(ppt.tagMode == 3 && ppt.dirMode == 1){
				...
				if(libpath.length == 1) {
					if(libpath[0] != "") arr[0] = "Root";
					else arr[0] = "Non-Library";
				}
	...
	this.sendItemToPlaylist = function(index) {
		...
		pidx = (plman.GetPlaylistName(0) == "Library") ? 1 : 0;
		var pidx_playing = -1;
	...
	this.init_swbtn = function(){
		switch(ppt.tagMode){
			...
			break;
			case 3:
			if (ppt.dirMode ==0) this.switch_btn = new button(images.sw_btn_n0, images.sw_btn_n0, images.sw_btn_n0, "Switch to library root");
			else this.switch_btn = new button(images.sw_btn_n1, images.sw_btn_n1, images.sw_btn_n1, "Switch to folder");
			break;
			default:
			this.switch_btn = new button(images.sw_btn_n0, images.sw_btn_n0, images.sw_btn_n0, "genre");
			break;
		}
	}
	this.init_swbtn();
	...
	this.reset_swbtn = function(){
		...
		switch(ppt.tagMode){
			...
			if (ppt.dirMode ==0) {
				this.switch_btn.img = Array(images.sw_btn_n0, images.sw_btn_n0, images.sw_btn_n0);
				this.switch_btn.Tooltip.Text = "Switch to library root";
			}else{
				this.switch_btn.img = Array(images.sw_btn_n1, images.sw_btn_n1, images.sw_btn_n1);
				this.switch_btn.Tooltip.Text = "Switch to folder";
			}
			break;
		}
	}
	...
	this.draw = function(gr) {
			...
			if (ay >= (0 - this.rowHeight) && ay < this.y + this.h) { // if stamp visible, we have to draw it
				...
				if (ppt.panelMode <= 1) { //(Art + bottom labels)
					...
					if (ppt.showAllItem && i == 0) {// && total > 1) { // aggregate item ( [ALL] )
						try {
								gr.GdiDrawText("All items", g_font_b, txt_color, ax + Math.round((aw - coverWidth) / 2), (coverTop + 5 + coverWidth), coverWidth, txth, lt_txt);
						} catch (e) {}
					} else {
						if (arr[1] == "?") {
							if (this.groups[i].count > 1) {
								...
								var album_name = (this.groups[i].tracktype != 3 ? "(Singles) " : "") + arr_t[0];
							};
						} else {
			...
			if (ppt.panelMode > 1) { 
				if (ppt.showAllItem) {
					if (this.selectedIndex == 0) {
						...
						gr.GdiDrawText("All items", g_font_b, txt_color, this.groups[0].x + 10, this.groups[0].y + coverWidth - ppt.botGridHeight, aw - 20, ppt.botGridHeight, lc_txt);
						...
						gr.GdiDrawText("All items", g_font, txt_color, this.groups[0].x + 10, this.groups[0].y + coverWidth - ppt.botGridHeight, aw - 20, ppt.botGridHeight, lc_txt);
					}
				}
			}
		...
		// draw top header bar
		var item_txt = nb_groups > 1? ["albums", "album artists", "artists", "folders", "directories", "genres"] : ["album", "album artist", "artist", "folder", "directory", "genre"];
		...
		try {
			...
			if(fb.IsPlaying && playing_title) gr.GdiDrawText("  |  "+ playing_title + " [playing] ", g_font, g_color_highlight, tx + source_width, 0, tw - source_width, ppt.headerBarHeight, lc_txt);
	...
	this.item_context_menu = function(x, y, albumIndex) {
		...
		_menu.AppendMenuItem(MF_STRING, 1010, "Refresh selected (reset cache)");
		if(pl_count > 1 && pl_count < 51){
			...
			if(addSep) _menu.AppendMenuItem(MF_STRING, 2000, "Send to a new playlist");
		} else _menu.AppendMenuItem(MF_STRING, 2000, "Send to a new playlist");
	...
	this.settings_context_menu = function(x, y) {
		...
		_menu0.AppendMenuItem((ppt.sourceMode == 1 && fb.IsLibraryEnabled()) ? MF_STRING : MF_DISABLED, 52, "Locked in Library playlist");
		...
		_menu1.AppendMenuItem(MF_STRING, 115, "Folder");
		_menu1.AppendMenuItem(MF_STRING, 116, "Library root directory");
		_menu1.AppendMenuItem(MF_STRING, 117, "Genre");
		switch (ppt.tagMode){
		case 1:
		...
		_menu2.AppendMenuItem(MF_STRING, 900, "Compact column mode");
		_menu2.AppendMenuItem(MF_STRING, 901, "Column mode");
		_menu2.AppendMenuItem(MF_STRING, 902, "Grid mode (no text)");
		...
		_menu2.AppendMenuItem(MF_STRING, 910, "Keep image aspect ratio");
		...
		_menu.AppendMenuItem(MF_STRING, 202, "Refresh (including cache)");
		_menu.AppendMenuSeparator();
		...
		case (idx == 52):
			...
			if(ppt.locklibpl) {
				if(!fb.IsLibraryEnabled()) {
					...
					g_active_playlist = 0;
					if (plman.GetPlaylistName(0) != "Library") {
...
function get_tagprop(){
	...
	case 1:
		...
		if(ppt.albumMode == 0){
			...
			ppt.tf_groupkey = fb.TitleFormat("$if2(%album%,Single) ## %title%");
	...
	case 2:
		...
		if(ppt.artistMode == 0){
			ppt.TFsorting = "%album artist% | $if(%album%,%date%,'9999') | %album% | %discnumber% | %tracknumber% | %title%";
			ppt.tf_groupkey = fb.TitleFormat("$if2(%album artist%,Unknown artist) ## %title%");
			...
			ppt.tf_groupkey = fb.TitleFormat("$if2(%artist%,Unknown artist) ## %title%");
	...
	case 4:
		...
		ppt.tf_groupkey = fb.TitleFormat("$if2(%genre%,Unknown genre) ## %title%");
		ppt.tf_autopl = "%genre%";
