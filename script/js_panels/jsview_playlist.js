...
var pidx = -1;
var tf_string = ["$if2(%album%,Single)", "$if2(%album artist%,Unknown artist)", "$if2(%artist%,Unknown artist)", "$if2(%genre%,Unknown genre)", "%directoryname%"];
		...
		if (!ppt.show_activepl){
			...
			if(ppt.title_type == 6){
				...
				if(path_start.length == 1) {
					if(path_start[0] != "") subject_start = "Library root";
					else subject_start = "Non-Library";
				} else {
					subject_start = path_start[0];
				}
				if(path_end.length == 1) {
					if(path_end[0] != "") subject_end = "Library root";
					else subject_end = "Non-Library";
				} else {
			...
			if(subject_start.toUpperCase() != subject_end.toUpperCase()) this.name = "All items";
			else {
				this.name = subject_start;
				if(ppt.title_type == 1 && this.name != "Single") this.show_tracknum = true;
			}
		} else {
	...
	this.on_mouse = function(event, x, y) {
		...
		case "dblclk":
			if (this.ishover && this.focusRow > -1) {
				if(plman.ActivePlaylist != pidx) fb.RunContextCommandWithMetadb("Play", this.rows[this.focusRow].metadb, 0);
...
//=================================== Main ================================================================
function init_btn(){
	if (ppt.show_activepl) btn_sw = new button(img_plsw_2, img_plsw_2, img_plsw_2, "Switch to library view");
	else btn_sw = new button(img_plsw, img_plsw, img_plsw, "Switch to active playlist");
}
...
function on_key_down(vkey) {
	...
	if (mask == KMask.none) {
		...
		case VK_RETURN:
			if (brw.rowsCount > 0) {
				if(brw.focusRow > -1) {
					if(plman.ActivePlaylist != pidx) fb.RunContextCommandWithMetadb("Play", brw.rows[brw.focusRow].metadb, 0);
