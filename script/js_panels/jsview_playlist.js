...
tf_string[1] = "$if2(%album%,Single)";
tf_string[2] = "$if2(%album artist%,Unknown artist)";
tf_string[3] = "$if2(%artist%,Unknown artist)";
tf_string[4] = "$if2(%genre%,Unknown genre)";
...
olist = function() {
	...
	this.get_list = function() {
		...
		if (!show_active_pl){
			...
			if(subject_start.toUpperCase() != subject_end.toUpperCase()) this.title = "All items";
			else this.title = subject_start;
		} else this.title = "Playlist: " + plman.GetPlaylistName(pidx);
		while (k < total) {
			metadb = this.list[k];
			this.id = k;
