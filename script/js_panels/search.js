...
function g_onSearch() {
	...
	if (ppts.source == 1) {
		...
		if (scopecases[ppts.scope]) {
			...
			plman.CreateAutoPlaylist(plIndex, "Search | " + s1, scopecases[1]+s3 + " OR " + scopecases[2]+s3 + " OR " + scopecases[3]+s3 + " OR " + scopecases[4]+s3 + " OR " + scopecases[5]+s3 + " OR " + scopecases[6]+s3, "", 0);
		};
