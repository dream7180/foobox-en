...
function g_onSearch() {
	...
	if (ppts.source == 1) {
		...
		if (scopecases[ppts.scope]) {
			plman.CreateAutoPlaylist(plIndex, "Search | " + s1, scopecases[ppts.scope] + s3, "", 0);
		} else {
			plman.CreateAutoPlaylist(plIndex, "Search | " + s1, scopecases[1]+s3 + " OR " + scopecases[2]+s3 + " OR " + scopecases[3]+s3 + " OR " + scopecases[4]+s3 + " OR " + scopecases[5]+s3, "", 0);
		};
...
function Show_Menu_Searchbox(x, y) {
	...
	_menu.AppendMenuItem(MF_STRING, 30, "Load internet radios");
	_menu.AppendMenuSeparator();
	...
	case (idx == ppts.historymaxitems + 60):
		...
		break;
	case (idx == 30):
		LoadRadio("Internet Radio", "https://mirror.ghproxy.com/https://raw.githubusercontent.com/fanmingming/live/main/radio/m3u/index.m3u");
		break;
	}
}
