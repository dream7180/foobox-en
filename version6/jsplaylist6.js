		...
		if (p.list) {
			if (p.list.count > 0) {
				...
				if (text_top.substr(0, 8) == "Search [") {
					...
					gr.GdiDrawText("No Result for \"" + search_text + "\"", g_font_blank, g_color_normal_txt, 0, 0 - z20, ww, wh, cc_txt);
					...
					if (clipboard.selection) {
						if (clipboard.selection.Count > 0) {
							try {
								...
								console.log("WSH Playlist WARNING: Clipboard can't be pasted, invalid clipboard content.");
							};
						};
					};
				};
...
function on_drag_drop(action, x, y, mask) {
	if (y < p.list.y) {
		...
		if (plman.ActivePlaylist > -1 && plman.IsPlaylistLocked(plman.ActivePlaylist)) {
			...
			plman.CreatePlaylist(count, "Dragged Items");
