...
function _panel() {
	...
	this.rbtn_up = (x, y, object) => {
		...
		this.m.AppendMenuItem(MF_STRING, 112, 'Click to create autoplaylist');
...
function _list() {
	...
	this.init = () => {
		this.add_meta = (f) => {
			for (let i = 0; i < f.MetaCount; i++) {
				...
				for (let j = 0; j < num; j++) {
					...
					if(_add){
						this.data.push({
							name : j == 0 ? name.slice(0,1).toUpperCase() + name.slice(1).toLowerCase() : '',
							value : value,
							url : url
						});
					}
		...
		this.add = (names) => {
			if (names) {
				let _names = [];
				if(names[0] == "PLAY_COUNT") _names = ['Played', 'First played', 'Last played', 'Added', 'Rating'];
				else _names = ['Album replay gain', 'Peak album amplitude', 'Track replay gain', 'Peak track amplitude'];
